...
router.post(
  ...
  /*validate(productValidation, {}, {}),*/ async (req, res) => {
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "INSERT INTO products SET productName = ?, sellerID = ?, productDescription=? , price= ?, inStock =? , visible = ?, freeShipping =?, productImg=?", //error is here
        [
          uploadData.productName,
          req.user.userID,
          uploadData.productDescription,
          uploadData.price,
          uploadData.inStock,
          uploadData.visible,
          uploadData.freeShipping,
          uploadData.productImg,
        ],
        (err, rows) => {
...
router.post(
  "/",
  /*isAuth,*/ upload.single("avatar"),
  /*validate(productValidation, {}, {}),*/ (req, res) => {
    pool.getConnection((err, connection) => {
      if (err) throw err;
