...
router.get("/order", isAuth, (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) throw err;
    connection.query(
      "SELECT * FROM orderdetails WHERE orderID = ? AND ( sellerID = ? OR buyerID = ?)",
      [req.params.orderID, req.user.userID, req.user.userID],
      (err, rows) => {
        connection.release();
        if (!err) {
          res.status(200).send(rows);
...
router.put("/cancel/:id", isAuth, (req, res) => {
  pool.getConnection((err, connection) => {
    ...
    connection.query(
      "UPDATE orderdetails SET orderStatus=? WHERE orderDetailID=? AND sellerID=?",
      [orderStatus, req.params.id, req.user.userID],
      (err, rows) => {
        connection.release();
        if (!err) {
...
router.put("/cancel/buyer/:id", isAuth, (req, res) => {
  pool.getConnection((err, connection) => {
    ...
    connection.query("SELECT orderStatus FROM orderdetails WHERE orderDetailID=?", [req.params.id], (err, rows) => {
      if (!err) {
        if (rows[0].orderStatus === pendingStatus) {
          connection.query(
            "UPDATE orderdetails SET orderStatus=? WHERE orderDetailID=? AND buyerID=?",
            [orderStatus, req.params.id, req.user.userID],
            (err, rows) => {
              connection.release();
              if (!err) {
                ...
                res.status(400).send("Bad request for canceling order.");
              }
            }
          );
        }
...
router.put("/confirm/:id", isAuth, (req, res) => {
  pool.getConnection((err, connection) => {
    ...
    //params is a request parameter in the url
    connection.query(
      "UPDATE orderdetails SET orderStatus=?, confirmationDate = CURRENT_DATE() WHERE orderDetailID=? AND sellerID=?",
      [orderStatus, req.params.id, req.user.userID],
      (err, rows) => {
        if (!err) {
          connection.query(
            "SELECT * FROM users LEFT JOIN orderdetails o on users.userID = o.sellerID OR users.userID = o.buyerID JOIN products p on o.productID=p.productID WHERE orderDetailID=?;",
            [req.params.id],
            (err, rows) => {
              connection.release();
              //the request has succeeded and a new resource has been created as a result.
              if (!err) {
                res.status(200).send(rows);
                //send mail to buyer
                Email.orderConfirmationMail(
                  rows[1].emailAddress,
                  rows[1].userFirstName,
                  rows[1].userLastName,
                  rows[0].userName,
                  rows[1].productName,
                  rows[1].quantityOrdered
                ).catch(console.error);
              } else {
                res.status(400).send("Bad request for seller & buyer data");
              }
