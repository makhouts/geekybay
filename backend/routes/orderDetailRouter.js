...
router.put("/cancel/buyer/:id", isAuth, (req, res) => {
    pool.getConnection((err, connection) => {
        ...
        connection.query("SELECT orderStatus FROM orderdetails WHERE orderDetailID=?", [req.params.id], (err, rows) => {
            if (!err) {
                //console.log(rows[0].orderStatus);
                if (rows[0].orderStatus === pendingStatus) {
                    connection.query("UPDATE orderdetails SET orderStatus=? WHERE orderDetailID=? AND buyerID=?", [orderStatus, req.params.id, req.user.userID], (err, rows) => {
                        connection.release();
                        if (!err) {
                            console.log(rows);
                            res.status(201).send(rows);
